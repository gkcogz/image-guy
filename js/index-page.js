    ...
    function renderApp() {
        ...
        if (allDone && appState.fileQueue.length > 0) {
            ...
            const formatList = [
                ...
                { id: 'webp', value: 'webp', label: 'WebP' },
                { id: 'heic', value: 'heic', label: 'HEIC' },
                { id: 'favicon-png', value: 'favicon-png', label: 'Favicon (PNG)' },
                { id: 'favicon-ico', value: 'favicon-ico', label: 'Favicon (ICO)' },
            ];

            const formatOptionsHTML = formatList.map(format => `
                <div class="radio-group">
                    <input type="radio" id="${format.id}" name="format" value="${format.value}" ${appState.selectedFormat === format.value ? 'checked' : ''}>
                    <label for="${format.id}">${format.label}</label>
                </div>
            `).join('');
